...
int tek_getsize(unsigned char *p) {
  static char header[15] = {0xff, 0xff, 0xff, 0x01, 0x00, 0x00, 0x00, 0x4f,
                            0x53, 0x41, 0x53, 0x4b, 0x43, 0x4d, 0x50};
  int size = -1;
...
static int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc,
                        int pb, int lp, int flags);

static int tek_lzrestore_tek5(int srcsiz, UCHAR *src, int outsiz,
                              UCHAR *outbuf) {
    ...
    if (bm->lt > 0) {
      if (--bm->lt == 0) {
        ...
        if ((bm->s = tek_rdget1(rd, &rd->probs.fchgprm[i * 2 + bm->s], 0x71, 0,
          ...
          tek_setbm5(
              bm, i,
              ((tek_rdget1(rd, rd->probs.tbmm, 0x74, 1, &rd->bm[2]) - 1) & 15) +
                  1);
        }
        bm->lt = bm->lt0;
      }
...
static int tek_getlen5(struct tek_STR_RNGDEC *rd, int m, int s_pos, int stk) {
  int i;
  if (tek_rdget1(rd, &rd->probs.lensel[m][0], 0x71, 0, rd->ptbm[3]) ^ stk)
    i = tek_rdget1(rd, rd->probs.pb[s_pos].lenlow[m], 0x73, 1, rd->ptbm[4]) & 7;
  else if (tek_rdget1(rd, &rd->probs.lensel[m][1], 0x71, 0, rd->ptbm[3]) ^ stk)
    ...
    if (i > 0) {
      if (i < 6 && stk == 0)
        i = tek_rdget1(rd, &rd->probs.lenext[(1 << i) - 2], i | 0x70, 1,
                       rd->ptbm[7]) -
            1;
      else
...
static int tek_decmain5(int *work, UCHAR *src, int osiz, UCHAR *q, int lc,
  ...
  int i, j, k, pmch, rep[4], s, pos, m_pos = (1 << pb) - 1,
                                     m_lp = (1 << lp) - 1;
  ...
  if (stk) {
    ...
    static UCHAR pt2[14] = {8,  8,  10, 11, 12, 12, 14,
    ...
    if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 0x71, 0, rd->ptbm[s > 0]) ^
      ...
      if (pmch == 0)
        *q = tek_rdget1(rd, &prb->lit[i], lit0cntmsk, 1, &rd->bm[24]) & 0xff;
        ...
        pmch = q[rep[0]];
        do {
          j += j + tek_rdget1(rd, &lit1[(i + j) << 1 | pmch >> 7], 0x71, 0,
                              rd->ptbm[2]);
          k--;
          if ((k & (lit0cntmsk >> 4)) == 0)
            bm++;
          if ((((pmch >> 7) ^ j) & 1) != 0 && k != 0) {
            j = tek_rdget1(rd, &prb->lit[i + j - 1], k | (lit0cntmsk & 0x70), j,
                           bm);
            break;
          }
          pmch <<= 1;
      ...
      if (tek_rdget1(rd, &prb->st[s].rep, 0x71, 0, rd->ptbm[13]) ^ stk) {
        ...
        if (j >= 4)
          j = 3;
        rep[0] = j =
            tek_rdget1(rd, prb->pslot[j], 0x76, 1, rd->ptbm[8 + (j == 3)]) &
            0x3f;
        if (j >= 4) {
          k = (j >> 1) - 1;
          rep[0] = (2 | (j & 1)) << k;
          if (j < 14)
            rep[0] |=
                tek_revbit(tek_rdget1(rd, &prb->spdis[j & 1][(1 << k) - 2],
                                      k | 0x70, 1, rd->ptbm[10 + (k >= 4)]),
                           k);
          else {
            if (stk == 0) {
              if (k -= 6)
              ...
              rep[0] |= tek_revbit(
                  tek_rdget1(rd, prb->algn, 0x76, 1, rd->ptbm[12]), 6);
            } else {
              rep[0] |= tek_rdget0(rd, k - 4, ~0) << 4;
              rep[0] |= tek_revbit(
                  tek_rdget1(rd, prb->algn, 0x74, 1, rd->ptbm[12]), 4);
            }
          }
        }
        rep[0] = ~rep[0];
        ...
        if (tek_rdget1(rd, &prb->st[s].repg0, 0x71, 0, rd->ptbm[13]) ^ stk) {
          i |= -1;
          if (tek_rdget1(rd, &prb->pb[s_pos].st[s].rep0l1, 0x71, 0,
                         rd->ptbm[13]) == 0) {
            s = s < 7 ? 9 : 11;
            goto skip;
          }
