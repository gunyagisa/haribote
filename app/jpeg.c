...
typedef struct {
  ...
  int base_img[64][64]; // ���摜 (
                        // [�����g��u��][�c���g��v��][���ʑ�(M/8)][�c�ʑ�(N/8)]
...
int decode0_JPEG(struct DLL_STRPICENV *env, int size, UCHAR *fp, int b_type,
                 UCHAR *buf, int skip);
...
int decode0_JPEG(struct DLL_STRPICENV *env, int size, UCHAR *fp0, int b_type,
                 UCHAR *buf, int skip) {
  JPEG *jpeg = (JPEG *)(((int *)env) + 128);
  jpeg->fp = fp0;
  jpeg->fp1 = fp0 + size;
...
#define INIT_ZTABLE(i, b0, b1, b2, b3)                                         \
  *(int *)&zigzag_table[i] = b0 | b1 << 8 | b2 << 16 | b3 << 24
